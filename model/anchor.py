...
def generate_anchors(P_h=None, P_w=None):
    '''
    generate anchor coordinates with 0 centered
    :param P_h:
    :param P_w:
    :return:
    '''
...
def shift(shape, stride, anchors):  # what is the shape
    shift_h = np.arange(0, shape[0]) * stride
    shift_w = np.arange(0, shape[1]) * stride

    shift_h, shift_w = np.meshgrid(shift_h, shift_w)
    shifts = np.vstack((shift_h.ravel(), shift_w.ravel())).transpose()  # flat array, all
    ...
    A = anchors.shape[0]
    K = shifts.shape[0]
    all_anchors = (anchors.reshape((1, A, 2)) + shifts.reshape((1, K, 2)).transpose((1, 0, 2))) # not even the same size, how broadcast
    all_anchors = all_anchors.reshape((K * A, 2))

    return all_anchors

class post_process(nn.Module):      # what is the shape and P_h here?
    def __init__(self, P_h=[2,6], P_w=[2,6], shape=[48,26], stride=8,thres = 8,is_3D=True): # shape 288/16 =  18 * 18,  stride use 16,  then each feature pt only has one anchor?
    ...
    def forward(self,heads,voting=False):
        ...
        for j in range(batch_size):
            ...
            reg = torch.unsqueeze(anchor,1) + regression    # reg coordinate
            ...
            if self.is_3D:
                ...
                P_keys.append(P_key)        # list add coord x,y, z(depth)
            else:
                P_keys.append(P_xy)
        return torch.stack(P_keys)  # (NxP) * 3
