...
import model.modules.PoseAttention as M
...
class PoseAttention(nn.Module):
	...
	def __init__(self, nChannels, nStack, nModules, numReductions, nJoints, LRNSize, IterSize, in_ch=3):
		...
		self.start = M.BnReluConv(in_ch, 64, kernelSize = 7, stride = 2, padding = 3)       # adapt to flexible channels
...
def get_pose_net(in_ch, out_ch, **kwargs):
	...
	model = PoseAttention(256, 2, 2, 4, out_ch, 1, 3, in_ch=in_ch)
